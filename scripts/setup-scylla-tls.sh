...
cat > "${SCYLLA_YAML}" <<EOF
# Generated by Globular installer (setup-scylla-tls.sh) — do not edit manually.
# cluster_name MUST match across all nodes; mismatch => topology coordinator
# rejects join with "Cluster name check failed". Day-1 join scripts also
# template "globular.internal" — keep this string in sync if you change it.
...
echo ""
# ─── Regression guard: scylla.yaml MUST contain cluster_name ─────────────
# If a future edit drops the cluster_name line, the install will succeed
# but Day-1 joins will fail with "topology coordinator rejected request to
# join the cluster: Cluster name check failed." This guard catches the
# template gap at install time, not after a failed cluster expansion.
if ! grep -qE "^cluster_name:\s*'globular.internal'" "${SCYLLA_YAML}"; then
    echo "✗ ERROR: setup-scylla-tls.sh wrote ${SCYLLA_YAML} without cluster_name='globular.internal'"
    echo "  This will cause Day-1 nodes to be rejected when joining the Scylla cluster."
    echo "  Fix: ensure the template in setup-scylla-tls.sh emits cluster_name."
    exit 1
fi
echo "  ✓ cluster_name guard: 'globular.internal' is present in scylla.yaml"
echo ""
