...
func (m *SystemdManager) ResetFailed(ctx context.Context, name string) error {
	if err := platform.ValidateServiceName(name); err != nil {
		return err
	}
	args := []string{"reset-failed", name}
	_, _, _ = m.run(ctx, args...)
	// Best-effort: ignore errors (unit may not be in failed state).
	return nil
}
